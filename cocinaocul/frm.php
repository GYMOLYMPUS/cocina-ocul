...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $fecha = $_POST['fecha'];
    $hora = $_POST['hora'];
    $llevaNinos = $_POST['llevaNinos'];
    $menuInfantil = isset($_POST['menuInfantil']) ? $_POST['menuInfantil'] : '';

    // Procesar datos de cada persona
    $menus = [];
    for ($i = 1; $i <= $personas; $i++) {
        $menus[] = [
            'condicion' => $_POST['condicion'.$i],
            'tipoComida' => $_POST['tipoComida'.$i]
        ];
    }

    // Procesar menú infantil si llevan niños
    if ($llevaNinos == "si" && $menuInfantil == "personalizado") {
        $condicionNino = $_POST['condicionNino'];
        $tipoComidaNino = $_POST['tipoComidaNino'];
    }

    // Aquí podrías guardar los datos en una base de datos o realizar otras acciones
    $mensajeExito = "¡La reserva fue creada con éxito!";
}
...
<body>
    <div class="form-container">
        ...
        <!-- Mensaje de éxito -->
        '. (!empty($mensajeExito) ? '<div class="success-message">'.$mensajeExito.'</div>' : '') .'
    </div>
